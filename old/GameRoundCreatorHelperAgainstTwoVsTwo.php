...
class GameRoundCreatorHelperAgainstTwoVsTwo
{
...
//
    protected ScheduleCyclePartAgainstOutput $cyclePartOutput;

    public function __construct(protected LoggerInterface $logger)
    {
        $this->cyclePartOutput = new ScheduleCyclePartAgainstOutput($logger);
    }



    public function assignHomeAways(ScheduleCyclePartAgainstTwoVsTwo $cyclePart): void {
        $nrOfPlaces = $cyclePart->cycle->sportSchedule->scheduleWithNrOfPlaces->nrOfPlaces;
        if( $nrOfPlaces % 4 === 0 ) {
            $this->assignHomeAways4N($cyclePart);
            return;
        }
        if( $nrOfPlaces % 4 === 1 ) {
            if( $nrOfPlaces === 5 ) {
            ...
            $this->assignHomeAways4NPlus1($cyclePart);
            return;
        }
        throw new \Exception('look for solutions 4%2 and 4%3');
    }

    private function assignHomeAways4N(ScheduleCyclePartAgainstTwoVsTwo $cyclePart): void {

        throw new \Exception('look for solutions 4%0');
    }

    private function assignHomeAways4NPlus1(ScheduleCyclePartAgainstTwoVsTwo $cyclePart): void {
